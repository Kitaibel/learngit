...
int main(int argc, char *argv[]) 
{
	...
	int choose_menu,continue_or_not=1;
...
void save_data(void)
{
	char stored_account[200][21]={};
	int stored_score[200],stored_top[200],count,i=0;
	FILE *fp = fopen("information.txt", "r+");
	while(fscanf(fp,"%s ",stored_account[i])!=EOF)
	{
		fscanf(fp,"%d %d\n",&stored_score[i],&stored_top[i]);
		i++;
	}
	FILE *fp2 = fopen("information.txt", "w+");
	for(count=0;count<i;count++)
	{
		if(!strcmp(now_account, stored_account[count]))
		{
			fprintf(fp2,"%s %d %d\n",now_account,score,top);
		}
		else
		{
			fprintf(fp2,"%s %d %d\n",stored_account[count],stored_score[count],stored_top[count]);
		}
	}
	system("cls");
